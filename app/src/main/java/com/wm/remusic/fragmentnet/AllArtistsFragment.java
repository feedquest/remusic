 ...
 */
public class AllArtistsFragment extends AttachFragment {
    ...
    class MAsyncTask extends AsyncTask {
        ...
        protected Object doInBackground(Object[] params) {
            ...
            JsonObject result = HttpUtil.getResposeJsonObject(BMA.GeDan.artistDan(next, 10));
            if (result == null) {
                return null;
            }
            //热门歌单
            JsonArray pArray = result.get("all_artists").getAsJsonArray();
            if (pArray == null) {
                return null;
            }
    ...
    private void loadData() {
        new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... params) {
                gson = new Gson();
                JsonObject result = HttpUtil.getResposeJsonObject(BMA.GeDan.artistDan(1, 10));
                if (result == null) {
                    return null;
                }
                //热门歌单
...
//                        intent.putExtra("albumart", info.getPic_300());
                        intent.putExtra("artistname", info.getTitle());
